    ...
    for fname in tqdm(sorted(os.listdir(embed_dir)), total=len(os.listdir(embed_dir))):
        ...
        for row in df_edge.itertuples():
            ...
            try:
                ...
                print("Something wrong loading embeddings.")
                continue
        ...
        # print(len(treat_idx), len(control_idx))
        if len(treat_idx) == 0 or len(control_idx) == 0:
            continue

        nn = NearestNeighbors(n_neighbors=1).fit(X[control_idx])
        dists, idxs = nn.kneighbors(X[treat_idx])
