...
from models import LinkPredictor, GNN, Identity, GraphWaveNet, AGCRN_Model, STGCN, VisionGNN, NodeVisionEncoder, GNNVisionMLP, GatedFusion, MLP
from models import MoE, GatedNetwork, Expert, Graphormer, GraphormerLayer
...
def main(args):
    ...
    if args.encoder == "none":
        ...
        model = STGCN(in_channels_node, in_channels_edge, hidden_channels=args.hidden_channels, 
                    ...
                    JK = args.jk_type, num_nodes=data.num_nodes).to(device)
    elif args.encoder == "vision_gnn":
        model = VisionGNN(
            node_attr_dim=in_channels_node,
            edge_attr_dim=in_channels_edge,
    ...
    elif args.encoder == "gnn_vision_mlp":
        model = GNNVisionMLP(
            node_attr_dim = in_channels_node,
            edge_attr_dim = in_channels_edge,
            gnn_hidden_dim = args.hidden_channels,
            vision_type = args.vision_type,
            state = args.state_name,
            gnn_type=args.gnn_type
        ).to(device)
    elif args.encoder == "gatedfusion":
        model = GatedFusion(
            node_attr_dim = in_channels_node,
            edge_attr_dim = in_channels_edge,
            gnn_hidden_dim = args.hidden_channels,
        ...
        model = MLP(
            node_attr_dim = in_channels_node,
            edge_attr_dim = in_channels_edge,
            mlp_hidden_dim = args.hidden_channels,
        ).to(device)
    elif args.encoder == "graphormer":
        model = Graphormer(in_channels_node, in_channels_edge, 
                hidden_dim=args.hidden_channels, 
                num_layers=args.num_gnn_layers, dropout=args.dropout,
                JK = args.jk_type).to(device)
    else:
    ...
    print(f"feature channels: {feature_channels}, in channels edge: {in_channels_edge}")
    if args.encoder == "gnn_vision_mlp" or args.encoder == "gatedfusion" or args.encoder == "mlp":
        predictor = LinkPredictor(in_channels=128*2 + in_channels_edge, 
                                hidden_channels=args.hidden_channels, 
                                out_channels=1,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_time_steps', type=int, default=2)
    # SupCon Arguments
    ...
    parser.add_argument('--vision_type', type=str, default="clip_image")
