...
class TrafficAccidentDataset:
    ...
    def __init__(self, state_name = "MA", data_dir = "./data",
        ...
        self.node_feature_name = f"{state_name}.npy" if node_feature_type == "centrality" or "vit" in state_name else f"{state_name}.npy"
    ...
    def load_monthly_data(self, year=2022, month=1):
        ...
        edge_feature_dir = os.path.join(self.data_dir, f"{self.state_name}/Edges/edge_features_{year}.pt")
        ...
        else: new_data.x = torch.zeros_like(normalized_node_features)

        if self.use_dynamic_edge_features:
            if new_data.edge_attr is None:
                new_data.edge_attr = normalized_edge_features
            else:
                # print("new_data.edge_attr: ",new_data.edge_attr.size())
                # print("normalized_edge_features: ",normalized_edge_features.size())
    ...
    def load_yearly_data(self, year=2022):
        ...
        # load the edge features 
        edge_feature_name = f"{self.state_name}/Edges/edge_features_{year}.pt"
    ...
    def load_static_edge_features(self):
        ...
        print("edge director: "+f"{self.state_name}/Edges/edge_features.pt")
    ...
    def load_static_network(self):
        ...
        print("------------------------------------")
        print("len(edge_index)::: ",len(edge_index[1]))
        print(adj)
    ...
    def compute_feature_mean_std(self, train_years):
        ...
        for year in train_years:
            for month in range(1, 13):
                ...
                if node_features!=None and edge_features!=None:
        ...
        for idx, year in enumerate(train_years):
            feature = all_node_features[idx]
            if feature is None:
                print(f"[Warning] node feature for year {year} is None!")
            # else:
...
def load_monthly_data(data, data_dir = "./data", state_name = "MA", num_negative_edges = 100000, year=2022, month=1):
    ...
    # load the edge features 
    edge_feature_dir = os.path.join(data_dir, f"{state_name}/Edges/edge_features_{year}.pt")
    if os.path.exists(edge_feature_dir):
        edge_feature_dict = torch.load(edge_feature_dir)
...
def load_yearly_data(data_dir = "./data", state_name = "MA", year=2022):
    ...
    '''
    # load the edge features 
    edge_feature_name = f"{state_name}/Edges/edge_features_{year}.pt"
