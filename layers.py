...
class GCNConv(MessagePassing):
    ...
    def __init__(self, in_channels_node, in_channels_edge, out_channels, aggr = "add", state_name="NV"):
    ...
    def message(self, x_j, edge_attr, norm):
        if edge_attr.shape[0] < x_j.shape[0]:
            edge_attr = torch.concat([edge_attr, torch.zeros(x_j.shape[0] - edge_attr.shape[0], edge_attr.shape[1], device=edge_attr.device)], dim=0)
